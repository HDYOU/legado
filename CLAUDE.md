...
1. **发现无关 bug/优化 → follow-up 报告**：任务过程中发现的 bug 或优化点，如果与当前 change 无关，不在本次修，而是作为 follow-up 报告单独提出。
2. **任务有歧义时选最直接的理解**：不要把其他可能的理解也一起做了，只实现最直接的那个理解。
3. **测试文件只在需要时提交**：任务没有明确要求、仓库惯例也不需要时，不提交测试文件。需要提交时，规模参照旁边已有的测试文件。
4. **沟通节奏**：开始执行任务之前，用一句话说明即将要做什么；工作过程中给出简短的进度更新；结尾写一段可以独立看懂的简短总结——发现了什么、做了什么、下一步是什么——让只看到最后一条消息的读者也能了解全貌。
